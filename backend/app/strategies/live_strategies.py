...
class RSIStrategy(BaseLiveStrategy):
    ...
    def check_signal(self, df):
        ...
        # আপনি চাইলে এখানে SELL কন্ডিশনও যোগ করতে পারেন
...
class MACDStrategy(BaseLiveStrategy):
    ...
    def check_signal(self, df):
        ...
        # Crossover Logic: Previous MACD < Signal AND Current MACD > Signal
...
class BollingerBandsStrategy(BaseLiveStrategy):
    """
    Bollinger Bands: Buy if price closes below Lower Band
    """
    def check_signal(self, df):
...
class SMACrossoverStrategy(BaseLiveStrategy):
    ...
    def check_signal(self, df):
        ...
        if prev_fast < prev_slow and curr_fast > curr_slow:
            return "BUY", f"SMA Cross ({fast_p} > {slow_p})", current_price
