    ...
    async def _execute_real(self, side: str, amount: float, price: float, order_type: str = "market", params: dict = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Precision handling failed (normal if markets not loaded): {prec_e}")

            logger.info(f"[REAL] Executing {final_order_type.upper()} {side} order on {self.exchange_id} for {amount} {symbol} at {final_price if final_order_type == 'limit' else 'Market'}")
            ...
            logger.error(f"[REAL] Order execution failed for {side} {amount} {self.pair} at {price}: {e}")
            return None
