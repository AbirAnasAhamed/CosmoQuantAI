...
class DynamicStrategyExecutor(BaseLiveStrategy):
    ...
    def check_signal(self, df: pd.DataFrame):
        ...
        try:
            ...
            for trigger in self.triggers:
                ...
                current_value = self._get_indicator_value(df, indicator, t_params)
    ...
    def _get_indicator_value(self, df, indicator, params):
        try:
            if indicator == 'RSI':
                ...
                # Default 14 if not in params? The UI schema didn't fully define periods for SMA node yet.
                # Assuming period 14 for now or if we add dynamic inputs later.
