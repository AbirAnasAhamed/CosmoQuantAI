...
@router.websocket("/ws")
async def websocket_dashboard_endpoint(websocket: WebSocket, db: Session = Depends(deps.get_db)):
    # AUTH NOTE: in a real app, you'd validate the token in the query param
    await manager.connect(websocket, "dashboard")
    
    # Ideally identify user from token. For now, we assume a single user or fetch the 'first' user
    # typically passed via ?token=... and decoded here. 
    # Since we can't easily inject Depends(get_current_user) in WS without param:
    # We will pick the user with ID=1 for this Demo/WebSocket context 
    # OR better: Assume the frontend sends an AUTH message first? 
    # For simplicity in this 'fix', let's use user_id=1 (Abir) as the target.
    user_id = 1 
    ...
    try:
        while True:
            ...
            # 3. Calculate 24h PnL (Simplified Logic)
            # Find a snapshot ~24h ago
            one_day_ago = datetime.now() - timedelta(days=1)
            ...
            if not snap:
                # Try oldest
                snap = db.query(PortfolioSnapshot).filter(PortfolioSnapshot.owner_id == user_id).order_by(PortfolioSnapshot.timestamp.asc()).first()
                
            prev_equity = snap.total_equity if snap else 25000.0
            
            # 24h Profit = Current - Prev (Assuming no deposits)
            ...
            # Direct send to this specific client
