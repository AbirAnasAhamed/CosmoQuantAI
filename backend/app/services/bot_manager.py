        ...
        try:
            ...
            if bot.strategy == "wall_hunter":
                ...
                if trading_mode == 'FUTURES':
                    ...
                    if config.get('liquidation_safety_pct'):
                        dist = config.get('liquidation_safety_pct')
                        logger.info(f"📐 Liq Safety Distance: {dist}%")
                        msg_lines.append(f"📐 Liq Distance: {dist}%")
    
                    if config.get('enable_ob_imbalance'):
                        ratio = config.get('ob_imbalance_ratio', 1.5)
                        logger.info(f"📊 Heatmap Imbalance: {ratio}x")
                        msg_lines.append(f"📊 Heatmap Imbalance: {ratio}x")
    
                logger.info(f"🎯 Target Spread: {config.get('target_spread', 0)}")
                msg_lines.append(f"🎯 Target Spread: {config.get('target_spread', 0)}")
                
                # Trigger Modes
                triggers = []
                if config.get('enable_wall_trigger', True):
                    triggers.append(f"Wall ({config.get('vol_threshold', 0)})")
                if config.get('enable_liq_trigger'):
                    is_btc = config.get('follow_btc_liq')
                    thresh = config.get('btc_liq_threshold' if is_btc else 'liq_threshold', 0)
                    liq_str = f"Liq ({'BTC' if is_btc else 'Local'}: ${thresh:,.0f}"
                    
                    # Add detailed flags
                    flags = []
                    if config.get('enable_liq_cascade'):
                        flags.append(f"Cascade:{config.get('liq_cascade_window', 5)}s")
                    if config.get('enable_dynamic_liq'):
                        flags.append(f"Dynamic:{config.get('dynamic_liq_multiplier', 1.0)}x")
                    if config.get('enable_ob_imbalance'):
                        flags.append(f"Imb:{config.get('ob_imbalance_ratio', 1.5)}x")
                ...
                if triggers:
                    ...
                    logger.info(f"🔔 Entry Triggers: {trigger_str}")
                    msg_lines.append(f"🔔 Entry Triggers: {trigger_str}")
                ...
                if config.get('enable_absorption'):
                    ...
                    logger.info(f"🧬 CVD Absorption: ON (Threshold: ${abs_thresh:,.0f}, Window: {abs_win}s)")
                    msg_lines.append(f"🧬 CVD Absorption: ON (${abs_thresh:,.0f} / {abs_win}s)")
                else:
