...
class ExchangeFlowService:
    ...
    async def fetch_latest_block_txs():
        ...
        async with aiohttp.ClientSession() as session:
            try:
                # 1. Get Latest Block (Full Transactions)
                url = f"https://api.etherscan.io/api?module=proxy&action=eth_getBlockByNumber&tag=latest&boolean=true&apikey={settings.ETHERSCAN_API_KEY}"
