    ...
    async def _update_dataframe(self, candle_data):
        ...
        try:
            ...
            if isinstance(candle_data, dict) and 'c' in candle_data: 
                ...
                # Note: KuCoin order is different!
            ...
            # Need to convert timestamp to datetime for Pandas if index requires it
            # self.df usually has integer index or datetime index? 
            # Looking at pandas_ta, usually expects index.
            
            # For simplicity, we append to the dataframe. 
            # Ideally, we check if the last row timestamp matches current candle. 
            # If so, update last row. If not, append new row.
            
            # Since we maintain a rolling window, we can just append and tail.
            
            # But wait! If we append on EVERY tick, we will have thousands of rows for the SAME minute.
            # We must only update the last row if it's the same candle, or append if it's a new one.
            
            # Since tracking exact candle boundaries is complex without full logic, 
            # We will assume this is a 'live' dataframe just for the current state.
            # A cheating way: Just update the last row always with the latest price.
            # Real strategies need 'closed' candles.
            
            # BETTER APPROACH:
            # Logic: If timestamp > last_candle_timestamp, append. Else update.
            
            if self.df.empty: return

            last_idx = self.df.index[-1]
            # Assuming DF index isn't the timestamp but 0..N, we need a 'timestamp' column or check index?
            # Let's assume 'close' is what matters most for calc.
            
            # Safest: Update last row.
            for col, val in new_row.items():
                if col in self.df.columns:
                    self.df.iloc[-1, self.df.columns.get_loc(col)] = val
            
            # Allow strategy to recalculate indicators (expensive?)
            # self.df.ta.rsi(append=True) ... 
            # If we strictly need indicators, we might need to re-run them occasionally
            
        except Exception as e:
            # print(f"DF Update Error: {e}")
            pass
