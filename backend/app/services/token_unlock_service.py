...
class TokenUnlockService:
    ...
    async def analyze_impact(self, event: TokenUnlockEvent) -> TokenUnlockEvent:
        """
        Calculates impact score and generates AI summary.
        """
        # 1. Quantitative Score (Simple logic: Amount USD vs Volume or Market Cap)
        # Assuming we don't have volume handy, we use a heuristic based on amount_usd
        # In a real app, we'd fetch 24h volume.
        
        # High impact if > $10M (arbitrary threshold for demo)
        if event.amount_usd > 10_000_000:
            event.impact_score = 90.0
        elif event.amount_usd > 1_000_000:
            event.impact_score = 60.0
        else:
            event.impact_score = 30.0
        ...
        if self.ai_client:
            try:
                prompt = (
                    ...
                    f"Current Circulating Supply: {event.circulating_supply_pct:.1f}%. "
                ...
                response = await self.ai_client.models.generate_content(
                    model="gemini-2.0-flash", # Using a fast model
                    contents=prompt
                )
                event.ai_summary = response.text.strip()
            except Exception as e:
                logger.error(f"AI Generation failed: {e}")
                event.ai_summary = "AI Analysis unavailable."
        else:
            event.ai_summary = "AI Analysis disabled (No API Key)."
