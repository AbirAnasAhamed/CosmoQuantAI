...
const BotLabHeader: React.FC<{ bots: ActiveBot[] }> = ({ bots }) => {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 staggered-fade-in">
             ...
             <div className="bg-white dark:bg-brand-dark rounded-xl p-4 border border-brand-border-light dark:border-brand-border-dark shadow-sm flex flex-col justify-center items-center text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-brand-darkest/50 transition-colors group">
...
const CreateBotModal: React.FC<{ 
    ...
    const inputClasses = "w-full bg-white dark:bg-slate-700/80 border border-brand-border-light dark:border-brand-border-dark rounded-md p-2 text-sm text-slate-900 dark:text-white focus:ring-brand-primary focus:border-brand-primary";
    
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-backdrop-fade-in" onClick={onClose}>
            <div className="bg-white dark:bg-brand-dark w-full max-w-lg rounded-2xl shadow-2xl flex flex-col animate-modal-content-slide-down" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-brand-border-light dark:border-brand-border-dark flex justify-between items-center">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">Launch New Bot</h2>
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-white">&times;</button>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Bot Name</label>
                        <input type="text" className={inputClasses} value={botName} onChange={e => setBotName(e.target.value)} placeholder="e.g. Alpha Seeker 1" autoFocus />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select className={inputClasses} value={selectedAsset} onChange={e => setSelectedAsset(e.target.value)}>
                             {['BTC/USDT', 'ETH/USDT', 'SOL/USDT'].map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                    </div>
                    <div className="pt-4 flex justify-end gap-3">
                        <Button type="button" variant="secondary" onClick={onClose}>Cancel</Button>
                        <Button type="submit" variant="primary">Launch Bot</Button>
                    </div>
                </form>
            </div>
        </div>
