...
export const ArbitrageScannerWidget: React.FC = () => {
    ...
    const fetchOpportunities = async () => {
        try {
            // Don't set loading true on poll to avoid flickering, only on initial mount or exchange switch
            const response = await fetch(`/api/v1/sentiment/arbitrage-opportunities?exchange_id=${selectedExchange}`);
            if (!response.ok) {
    ...
    useEffect(() => {
        setLoading(true); // Show loading when switching exchange
