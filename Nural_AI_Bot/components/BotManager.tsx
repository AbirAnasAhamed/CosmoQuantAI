...
import { Bot, Play, Pause, Square, Plus, Activity, TrendingUp, DollarSign, BarChart3, Crown, ShieldAlert, Lock, Unlock, Cpu, Network, CheckCircle2, XCircle, AlertTriangle, Share2, Search, ArrowUpRight, CloudLightning } from 'lucide-react';
    ...
    <div className="space-y-8 relative">
      ...
      <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {bots.map((bot) => (
              <div key={bot.id} className={`bg-omni-panel border rounded-xl overflow-hidden hover:border-slate-500 transition-all duration-300 group relative ${
                  bot.id === 'BOT-OMEGA' ? 'border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-slate-700'
              }`}>
                ...
                <div className={`h-1 w-full ${
                    bot.status === 'RUNNING' ? 'bg-omni-success' : 
                  ...
                  <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`p-3 rounded-lg ${
                            bot.id === 'BOT-OMEGA' ? 'bg-amber-500/20 text-amber-400' :
                      ...
                      <div className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1.5 ${
                        bot.status === 'RUNNING' ? 'bg-omni-success/10 text-omni-success border-omni-success/30' :
                        bot.status === 'PAUSED' ? 'bg-omni-warning/10 text-omni-warning border-omni-warning/30' :
                        'bg-slate-700 text-slate-400 border-slate-600'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${
                            bot.status === 'RUNNING' ? 'bg-omni-success animate-pulse' :
                            bot.status === 'PAUSED' ? 'bg-omni-warning' :
                            'bg-slate-500'
                        }`}></span>
                        {bot.status}
                 ...
                 <div className="p-6 overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {swarmConfig.map((agent) => (
                            <div key={agent.id} className={`p-4 rounded-lg border flex items-center justify-between transition-all ${
                                agent.isActive 
                                ...
                                <button 
                                    ...
                                    className={`w-12 h-6 rounded-full p-1 transition-colors relative ${
                                        agent.isActive ? 'bg-indigo-500' : 'bg-slate-700'
                                    }`}
                                >
                                    <div className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${
                                        agent.isActive ? 'translate-x-6' : 'translate-x-0'
                                    }`}></div>
                                </button>
                            </div>
                        ))}
