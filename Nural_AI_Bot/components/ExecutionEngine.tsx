...
import { Shield, PlayCircle, StopCircle, Zap, BarChart2, AlertTriangle, RefreshCcw, Calculator, Sliders, Lock, ArrowRightLeft, FileText, AlertOctagon, Brain, ChevronDown, ChevronUp, Microscope, Activity, Crosshair, Globe, Layers, Clock, TrendingDown, BookOpen, MoveDown, MoveUp, Gauge, Flame, Percent, Target, ArrowUpDown, ArrowUp, ArrowDown, Info, TrendingUp, Download, Compass } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  {['LIMIT', 'MARKET', 'STOP'].map((type) => (
                      <button 
                        ...
                        className={`flex-1 py-2 text-xs font-bold rounded transition-all ${
                            orderType === type ? 'bg-blue-500 text-white shadow' : 'text-slate-400 hover:text-white'
                        }`}
                      >
                          {type}
                      </button>
            ...
            <div className="bg-omni-panel border border-slate-700 rounded-xl p-6 flex flex-col justify-between shadow-lg h-full relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <button 
                        ...
                        className={`w-full py-6 rounded-xl font-black text-lg flex items-center justify-center gap-3 transition-all shadow-xl transform active:scale-95 border border-white/10 ${
                            isRunning 
        ...
        <div className="lg:col-span-4 bg-omni-panel border border-slate-700 rounded-xl p-6 flex flex-col">
                ...
                <div className="grid grid-cols-3 gap-1 p-1 bg-slate-800 rounded-lg border border-slate-700">
                    {[
                        ...
                        <button
                            ...
                            className={`flex flex-col items-center justify-center py-2 rounded transition-all ${
                                executionStrategy === strat.id 
            ...
            <div className="space-y-4 flex-1">
                ...
                <div className={`bg-slate-800 rounded-lg p-3 border transition-all duration-300 relative overflow-hidden group ${
                    isFlashingRed ? 'border-omni-danger animate-pulse shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 
                    ...
                    <input 
                        ...
                        className={`w-full h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer relative z-10 ${
                        isCriticalRisk ? 'accent-omni-danger' : isHighRisk ? 'accent-omni-warning' : 'accent-blue-500'
                        }`}
                    />
                ...
                <React.Fragment key={trade.id}>
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-right font-bold ${
                      (trade.pnl || 0) > 0 ? 'text-omni-success' : (trade.pnl || 0) < 0 ? 'text-omni-danger' : 'text-slate-500'
                    }`}>
